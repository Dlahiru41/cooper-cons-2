...
function createInteractiveClientSection() {
    // Find the existing clients grid
    const clientsLogoGrid = document.querySelector('.clients-logo-grid');

    if (!clientsLogoGrid) return;

    // Client data - replace image elements with structured client data
    const clients = [
        {
            name: "Prime Lands",
            description: "Leading property developer in Sri Lanka with numerous luxury housing projects across the island."
        },
        {
            name: "Sumana Constructions",
            description: "Specialized in commercial building construction with over 15 years of industry experience."
        },
        {
            name: "L&T Construction",
            description: "One of South Asia's largest construction companies with expertise in infrastructure development."
        },
        {
            name: "LECO",
            description: "Lanka Electricity Company providing power supply infrastructure across urban and rural areas."
        },
        {
            name: "SAW Engineering",
            description: "Engineering consultancy firm specializing in structural design and technical evaluations."
        },
        {
            name: "Sanken Construction",
            description: "Premier high-rise construction specialists with landmark projects in Colombo."
        },
        {
            name: "RS Projects",
            description: "Innovative project management firm focused on sustainable construction practices."
        }
    ];

    // Create new HTML structure
    const newClientSection = document.createElement('div');
    newClientSection.className = 'clients-text-grid';

    // Add client items
    clients.forEach(client => {
        const clientItem = document.createElement('div');
        clientItem.className = 'client-item';
        clientItem.setAttribute('data-client', client.name);

        // Add the client name as the main text element
        const clientName = document.createElement('h3');
        clientName.className = 'client-name';
        clientName.textContent = client.name;

        // Add the description that will be revealed on interaction
        ...
        // Add a "Learn more" text that will be shown initially
        const learnMore = document.createElement('span');
        learnMore.className = 'learn-more';
        learnMore.textContent = 'Learn more';

        // Assemble the client item
        clientItem.appendChild(clientName);
        clientItem.appendChild(clientDescription);
        clientItem.appendChild(learnMore);

        // Add to the main container
        newClientSection.appendChild(clientItem);
    });

    // Replace the existing grid with the new one
    clientsLogoGrid.parentNode.replaceChild(newClientSection, clientsLogoGrid);

    // Add CSS styles for the new client section
    const styleSheet = document.createElement('style');
    styleSheet.innerHTML = `
    /* Interactive Clients Section Styles */
    .clients-text-grid {
      display: grid;
    ...
    .client-item.active {
      ...
      background-color: rgba(112, 154, 213, 0.05);
    }
    ...
    .client-item.active .client-description {
      height: auto;
      opacity: 1;
      transform: translateY(0);
    }
    ...
    /* Hover effect - show a slight hint of the description */
    .client-item:hover:not(.active) .client-name {
      transform: translateY(-5px);
    }
  ...
  `;
    ...
    // Add automatic rotation for showcasing
    let currentActiveIndex = -1;
    const autoRotateInterval = 5000; // 5 seconds

    function rotateActiveClient() {
        currentActiveIndex = (currentActiveIndex + 1) % clientItems.length;

        // Deactivate all
        clientItems.forEach(item => item.classList.remove('active'));

        // Activate next
        clientItems[currentActiveIndex].classList.add('active');

        // Scroll into view if needed
        if (!isInViewport(clientItems[currentActiveIndex])) {
            clientItems[currentActiveIndex].scrollIntoView({
                behavior: 'smooth',
                block: 'center'
            });
        }
    ...
    // Start the rotation
    const rotationTimer = setInterval(rotateActiveClient, autoRotateInterval);

    // Stop rotation when user interacts with any client item
    clientItems.forEach(item => {
        item.addEventListener('click', function() {
    ...
    // Initial activation
    setTimeout(rotateActiveClient, 1000);
}
...
// Initialize the interactive client section
document.addEventListener('DOMContentLoaded', function() {
    createInteractiveClientSection();
});